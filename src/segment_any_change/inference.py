...
def load_default_sam_params():
    ...
    return ExperimentParams(**(exp_params | seganychange_params | sam_params | dir_params))

def load_partial_ds(ds_name: str, dtype, indices: Sequence[int]=None):
    ds = BiTemporalDataset(name=ds_name, 
                           dtype=dtype, 
...
def infer_on_sample(A_path: str, 
                    B_path: str, 
                    label_path: str, 
...
def partial_inference(model: Any=None,
                ds_name: str="levir-cd", 
                dtype: str="test", 
                batch_size: int=None,
                indices: Sequence[int]=None) -> List[Dict]:
