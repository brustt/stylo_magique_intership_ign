...
def prepare_prompts(
    ...
    original_size: Optional[Tuple[int, int]] = IMG_SIZE,
...
class DefaultTransform:
    ...
    def __call__(self, sample: Dict) -> Dict:
        ...
        return {"img_A": img_A, "img_B": img_B, "label": label, "index": index}
...
def prepare_image(input_image: np.ndarray):
    ...
    input_image_torch = preprocess_image(input_image_torch)
    return input_image_torch
...
def normalize_img(
    x: torch.Tensor,
    pixel_mean: torch.Tensor = PX_MEAN,
    pixel_std: torch.Tensor = PX_STD,
) -> torch.Tensor:
