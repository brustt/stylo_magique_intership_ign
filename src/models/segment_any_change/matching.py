...
class BitemporalMatching:
    def __init__(
        ...
        **sam_kwargs,
        ...
        self.mask_generator = SegAnyMaskGenerator(
            model=load_ckpt_sam(network, sam_kwargs.get("sam_ckpt_path")), **sam_kwargs
        )
    ...
    def run(self, batch: Dict[str, torch.Tensor], **params) -> Any:
        ...
        imgs_embedding_A = get_img_embedding_normed(
            self.mask_generator.model, ImgType.A
        )
        imgs_embedding_B = get_img_embedding_normed(
            self.mask_generator.model, ImgType.B
        )
        ...
        for i in range(batch_size):
            ...
            # t -> t+1
            # ci : N
            x_t_mA, _, ci = temporal_matching_torch(img_emb_A, img_emb_B, masks_A)
            # t+1 -> t
            # ci1 : N
            _, x_t1_mB, ci1 = temporal_matching_torch(img_emb_A, img_emb_B, masks_B)
            # 2, max(NA,NB) x H x W
            ...
            masks_logits = stack_tensor_from_sam_output(
                img_anns_curr, key="masks_logits"
            )
        ...
        masks = pad_sequence(
            [elem["masks"] for elem in batch_filtered], batch_first=True
        )
        masks_logits = pad_sequence(
            [elem["masks_logits"] for elem in batch_filtered], batch_first=True
        )
        iou_preds = pad_sequence(
            [elem["iou_preds"] for elem in batch_filtered], batch_first=True
        )
        ci = pad_sequence([elem["ci"] for elem in batch_filtered], batch_first=True)
        proposal_emb = pad_sequence(
            [elem["proposal_emb"] for elem in batch_filtered], batch_first=True
        )
...
def stack_tensor_from_sam_output(
    img_anns: List[Dict], key: str = "masks"
) -> torch.Tensor:
