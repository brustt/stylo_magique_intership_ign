...
class SegAnyPrompt:
    def __init__(
        self,
        matching_engine: Union[BitemporalMatching, Any] = None,
        network: BiSam = None,
        **params
    ):

        if not any([matching_engine, network]):
            raise RuntimeError("Please provide at leat matching engine or SAM model")
    ...
    def __call__(self, batch: Dict[str, torch.Tensor]):
        ...
        items_change = self.matching_engine(grid_batch)

        # query prompt
        query_res = QueryPointMecanism(
            items_change,
            model=self.matching_engine.mask_generator.model,  # sam
            th_sim=self.params["th_sim"],
        ).run(batch)
...
class QueryPointMecanism:
    ...
    def get_best_masks(
        self, masks: torch.Tensor, ious: torch.Tensor
    ...
    def run(self, batched_input: Dict) -> torch.Tensor:
        ...
        for i in range(batch_size):
            ...
            sim_scores = self.compute_cross_sim(
                masks_embedding[i, ...], self.items_change["proposal_emb"][i, ...]
            )
            ...
            data, th = thresholding(
                data, attr="sim", method=self.th_sim, filtering_type=FilteringType.Sup
            )
